...
public class LoadDataStage extends AbstractDistStage {
   ...
   @Property(doc = "Specifies if the requests should be explicitly wrapped in transactions. By default" +
         "if the service and cache is support the transactions and transactionSize > 0," +
   ...
   public DistStageAck executeOnSlave() {
      ...
      if (useTransactions != null && useTransactions) {
   ...
   protected List<Loader> startLoaders() {
      ...
      for (int i = 0; i < numThreads; ++i) {
         ...
         if (this.useTransactions == null) {
            ...
            useTransactions = transactional != null && transactionSize > 0
                  && transactional.getConfiguration(cacheName) == Transactional.Configuration.TRANSACTIONAL;
         } else {
