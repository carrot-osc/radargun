            ...
            try {
               ...
               for (DelayedRemove delayedRemove : delayedRemoves.values()) {
                  try {
                     ...
                     if (delayedRemoveAttemptsMap.get(delayedRemove.keyId) > manager.getLogLogicConfiguration().getMaxDelayedRemoveAttempts()) {
                        log.errorf("Maximum number of delayed remove attempts on key %s attained, reporting.", delayedRemove.keyId);
